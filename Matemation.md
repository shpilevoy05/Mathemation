...
### Экономика внешнего вида и правки первой примерки (2026-08-02)

22. **Магазин продаёт две разные вещи.** Косметика надевается и живёт в
    инвентаре; расходник (заморозка стрика, ускоритель опыта) срабатывает в
    момент покупки и покупается повторно. Заморозка закрывает пропущенный
    период и сохраняет серию — ровно за это её и берут; ускорители не
    складываются, действует самый сильный, иначе экономика ломается
    покупкой десяти штук подряд.
23. **Косметика видна в интерфейсе.** У предмета есть машинный код, по
    которому кабинет рисует аватар, рамку и тему. Тем четыре: ночь, рассвет,
    лес и графит — каждая самостоятельный набор токенов, а не оттенок.
    Неизвестный код игнорируется: витрина не должна ломать страницу.
24. **Апселлы отделены от тарифа.** Дополнительная проверка эксперта и пакет
    подсказок наставника не создают подписку и не версионируются как цена —
    это конечное число единиц к текущему периоду.
25. **Урок закрывается решёнными задачами.** Точка урока на дорожке
    отмечается выполненной, когда решены все задачи темы; порог освоения
    закрывает точку практики. Отчёт родителя считает занятия по факту
    закрытия пункта, а не по сроку в плане.
26. **Вердикт эксперта возвращается ученику.** Работа, отправленная на
    доработку, видна в «Отработке» с баллом, комментарием и формой повторной
    загрузки: раньше она не появлялась нигде.

### Занятие как последовательность этапов (2026-08-02)

27. **Занятие проходится тремя этапами:** материал (видео и конспект для
    скачивания), задачи по теме, отработка ошибок темы. Шкала сверху
    показывает не только сколько сделано, но и где ученик находится:
    текущим считается первый незакрытый этап.
28. **Материал закрывает сам ученик.** Задачи и отработку можно посчитать
    по попыткам и полке ошибок, а «понял материал» знает только он — это
    единственное, что хранится в `LessonProgress`.
29. **Переход между этапами показывает результат работы.** Ответ на задачу
    возвращает рост освоения, XP и сигмы, и карточка перехода называет эти
    числа. Награда меряется до и после действия в API, а не собирается по
    кускам в интерфейсе — иначе интерфейс врёт.
30. **Конспект — файл, а не только раскрывающийся блок.** Markdown читается
    где угодно, печатается и остаётся редактируемым; отдаётся только
    ученику, потому что это часть оплаченного продукта.

### Второй визуальный слой (2026-08-08)

31. **Интерфейс переехал на макет «Визуал проекта: анимации и графика».**
    Навигация ушла в тёмный рельс `#141A33`, рабочая область осталась
    светлой с точечной сеткой. Шрифты — Onest для заголовков и чисел,
    Manrope для текста; цифры набираются табличными знаками того же
    гротеска, потому что моноширинный рядом с Onest читался как код.
32. **Иконки — общий спрайт, а не эмодзи.** Один `partials/icons.html`
    на весь кабинет: эмодзи выглядели по-разному в каждой системе и не
    принимали цвет состояния.
33. **Кнопка нажимается.** Нижняя тень уезжает под палец — это единственная
    анимация, которую видно на каждом экране, и она отличает тренажёр от
    формы. Всё остальное движение (пламя серии, орбиты в герое, рост
    полос) выключается под `prefers-reduced-motion`.
34. **Кабинет начинается с ответа на «что сегодня».** Герой держит старт,
    прогноз и цель на одной шкале с пламенем-ползунком, под ним полоса
    задания дня с остатком времени, дальше четыре плитки показателей со
    спарклайнами и полосой серии по дням недели.

### Подготовка к коммерческому запуску (2026-08-09)

35. **План переоценивается по мере прогресса.** Закрытая тема меняет и
    пороги пререквизитов, и выгоду остальных тем, поэтому очередь
    пересобирается сразу после закрытия пункта и ночью для всех активных
    планов. Сделанное не трогается: пересобирается только незакрытая часть.
36. **Расписание считается в часах и знает про дату экзамена.** Тема на
    четыре часа занимает вдвое больше недельного бюджета, чем тема на два, а
    план не уходит за дату экзамена: при нехватке недель тот же объём
    сжимается в оставшиеся.
37. **Срочное стоит первым.** Возвращённая на отработку тема получает срок
    «завтра» и первый порядковый номер: раньше срок и порядок противоречили
    друг другу. Просроченные пункты переносятся на сегодня, а не копятся.
38. **Лимит частоты — часть продукта, а не инфраструктуры.** Подсказка
    наставника стоит денег, ответ пишется в append-only лог, покупка меняет
    баланс: у каждого из них свой лимит. Вход и регистрация по коду —
    формы, поэтому у них свой счётчик в общем кеше; логин считается и по
    адресу, и по логину сразу.
39. **Действия бэкофиса пишутся в тот же лог событий**, что и учебные:
    публикация урока, смена цены, выдача домашки, начисление сигм,
    отключение ученика и возврат денег — с автором.
40. **Доступ к платному отделён от платежа.** Платёж отвечает «заплатили ли»,
    гейт — «можно ли открыть занятие прямо сейчас», и между ними стоят
    пробный период, бесплатная часть и роли сотрудников. Разрез
    «бесплатно/платно» живёт в настройках (`FREE_FEATURES`, `TRIAL_DAYS`), а
    сам гейт включается отдельно (`BILLING_ENFORCED`): пока эквайринга нет,
    закрывать доступ нечем.
41. **Оплата признаётся по подписи провайдера, а не по словам клиента.**
    Колбэк приходит из интернета без сессии, поэтому проверяется HMAC сырого
    тела на общем секрете; повторная доставка безопасна по
    `idempotency_key`, а неизвестный платёж подтверждается 200, чтобы
    провайдер не ретраил вечно.
42. **Ссылка на видео выдаётся, а не публикуется.** Видео — единственная
    часть контента, которую можно унести одной ссылкой, поэтому в HTML её
    нет: плеер запрашивает короткоживущую ссылку, запрос проходит гейт
    подписки и виден в логах. Хостинг спрятан за адаптером, а список
    разрешённых хостов встраивания закрытый — iframe исполняется в контексте
    страницы ученика.
43. **Бэкофис требует второй фактор.** Сотрудник видит чужие персональные
    данные, публикует контент и двигает деньги, поэтому пароля ему мало.
    Требование проверяется перед каждым запросом, а не во вьюхе входа: входов
    в систему несколько, а правило должно быть одно. Ученику и родителю фактор
    не навязываем — чужих данных он не открывает, а порог входа поднимает.
    Потеря телефона лечится резервными кодами, а полная потеря — командой на
    сервере с записью в журнал: иначе включение фактора однажды заблокирует
    команду целиком.
44. **Папка навыков — не навык.** В методической разметке есть узлы вроде
    «Отбор корней» и «ОДЗ»: у них есть имя и место в карте, но нет своей
    практики. Папка не попадает ни в план, ни на дорожку, ни в прогноз, своего
    освоения не имеет — оно считается по детям, способом (среднее или минимум),
    который выбирает методист. Иерархия ровно на один уровень: папка в папку не
    вкладывается.
45. **Связь связи рознь.** Обязательная закрывает тему до освоения
    предшественника, поддерживающая только объясняет порядок. Считать
    поддерживающие воротами нельзя: в разметке их треть, и граф захлопнулся бы
    на темах, которые решаются без них. Ацикличность требуется только от
    обязательных — «одно помогает понять другое» в обе стороны это нормальное
    методическое утверждение.
50. **Арена соревнует, но не учит вместо плана.** Партии против друга или
    бота дают короткий цикл «сыграл — увидел результат», которого нет у плана
    на полгода. Ответы пишутся в журнал попыток, но освоение тем не двигают и
    ошибок на полку не заводят: под таймером человек ошибается от спешки, а не
    от незнания, и учить план по этому нельзя. Награда небольшая и с дневным
    потолком — арена возвращает к занятиям, а не заменяет их.
51. **Разбор партии обязателен, отработка — нет.** Ошибки арены не идут на
    полку повторов, но после партии ученик видит каждый вопрос со своим и
    правильным ответом: без разбора партия превращается в бросок кубика.
52. **Случайный соперник подбирается по уровню.** У игрока есть рейтинг,
    стартующий от прогноза балла и меняющийся только в случайных партиях двух
    людей: о бота и о друга рейтинг не набивается. Очередь ищет сначала
    равного, потом ближайшего — окно расширяется со временем ожидания, а
    через минуту предлагается бот того же уровня, потому что ждать вечно
    скучнее, чем играть.
53. **Соперник-бот честен.** Его прогон разыгрывается один раз при создании
    партии по той же IRT-модели, которой платформа оценивает шансы ученика, и
    не зависит от того, как играет человек. Подстраивающийся бот — это подкрутка,
    а мы обещаем честную оценку.
49. **Календарь — это план на сетке дат, а не второе расписание.** В нём
    видны занятия, повторы, дедлайны домашек и дата экзамена, а перенос
    двигает сам пункт плана и пишется в журнал изменений. Назад в прошлое и за
    дату экзамена перенос не делается: первое — подделка истории, второе —
    обещание времени, которого нет.
48. **Освоение двигает наблюдаемый шаг, а не балл за задачу.** У задачи
    второй части есть эталонный путь из упорядоченных шагов, и эксперт
    отмечает каждый: выполнен, выполнен с ошибкой, не выполнен. Балл говорит,
    сколько потеряно, отметки — где именно. Три правила разметки при этом
    обязательны: нет действия — нет обновления (пропущенный шаг не понижает
    навык), роль и сила сигнала независимы (вспомогательный шаг может дать
    сильное свидетельство), свидетельство идёт самому конкретному узлу и не
    переносится ни на предков, ни на предшественников. Пока путь не размечен,
    работает прежнее поведение по тегам задачи.
47. **Ответ проверяется по смыслу, а не по записи.** Ответ второй части —
    множество точек: порядок серий, буква параметра и форма записи значения не
    имеют, `π/4+πk/2` равно паре `π/4+πk`, `3π/4+πk`. И третье состояние, кроме
    «верно/неверно»: «не разобрал запись». Описка не заводит ошибку на полку и
    не двигает освоение — ученика просят переписать. На пробнике и диагностике
    переспросить некого, поэтому там нечитаемый ответ не приносит балла, как на
    экзамене.
46. **Разметка живёт в коде и применяется идемпотентно.** Книга методистов
    меняется примерно раз в год, поэтому она не загружается файлом через
    бэкофис: данные лежат модулем в репозитории, применяются командой по коду
    навыка и проверяются в CI теми же проверками, что раньше делались глазами.
    Загрузка обновляет на месте и никогда не удаляет молча: исчезнувший из
    книги навык попадает в отчёт, а удаляется только явно и только если за
    него не держатся задачи и история учеников. Часы, вес и порядок разметка
    не трогает — их ведёт методист в панели, и перезапись затёрла бы его
    работу.
